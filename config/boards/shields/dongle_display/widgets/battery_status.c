...
static uint8_t battery_image_buffer[ZMK_SPLIT_BLE_PERIPHERAL_COUNT]
    [LV_CANVAS_BUF_SIZE(5, 8, LV_COLOR_FORMAT_GET_BPP(LV_COLOR_FORMAT_L8), LV_DRAW_BUF_STRIDE_ALIGN)];
...
static void draw_battery(lv_obj_t *canvas, uint8_t level) {
    ...
    if (level > 90) {
        ...
        area = (lv_area_t){1, 2, 3, 2};
        lv_draw_rect(&layer, &rect_fill_dsc, &area);
    } else if (level > 50) {
        area = (lv_area_t){1, 2, 3, 3};
        lv_draw_rect(&layer, &rect_fill_dsc, &area);
    } else if (level > 30) {
        area = (lv_area_t){1, 2, 3, 4};
        lv_draw_rect(&layer, &rect_fill_dsc, &area);
    } else if (level > 10) {
        area = (lv_area_t){1, 2, 3, 5};
        lv_draw_rect(&layer, &rect_fill_dsc, &area);
    } else {
        area = (lv_area_t){1, 2, 3, 6};
        lv_draw_rect(&layer, &rect_fill_dsc, &area);
    }
